...
"""
# Character class for player character and enemies
class Character:
...
class Inventory:
    def __init__(self):
        self.items = []
# Adding item to inventory message
    def add_item(self, item):
        self.items.append(item)
        print(f"You found a {item}.")
# Using item from inventory message removes it from player inventory
...
#  Message to player to show their inventory status
...
# Room class that houses descriptions and items
class Room:
...
# Game Logic
# Dice roll of a 20 sided die for random outcomes for checks
def dice_roll():
    """
...
def create_character():
    ...
    # Stat bonuse sfor attributes based on race choice
    if race_choice == 1:  # Earthling race 
        attributes["Luck"] += 3
        attributes["Attack"]-= 1

    elif race_choice == 2:  # Martian race
        attributes["Attack"] += 2
        attributes["Health"]-= 2

    elif race_choice == 3:  # Gorbling race 
        attributes["Luck"] += 3
        attributes["Attack"] -= 3

    elif race_choice == 4: #Cthuleon race
        attributes ["Attack"] += 5
        attributes ["Luck"] -= 5
#  Logic to ensure that player can allocate stats or finish to proceed
...
# Intro messaging and how to play
def intro():
    """
    Function to display the introduction and game instructions.
    """
    print(INTRO_ART)
    print("Welcome to Space RPG The Void!")
...
def main():
    ...
    action = player_input("What do you want to do? (search, exit, talk, attack, inventory): ").lower()
